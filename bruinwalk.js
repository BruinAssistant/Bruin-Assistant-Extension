...
const observer = new MutationObserver((mutationsList, observer) => {
    ...
    for (const mutation of mutationsList) {
        if (mutation.target.className !== "hide-small") {
            trigger = true;
            break;
        }
    }
...
// need to also work on "search for class" tab
function findClassInst(){
    ...
    // for (const oneClass of uclaClasses){
            ...
            if(allInstName[j] != "..."){
                ...
                getSearchResult(fullCourseName, allInstName[j], (instUrl => {
                    if (instUrl != null){
                        ...
                        bruinwalkLink.innerText = "Click to bruinwalk";
                        courseTable[7].appendChild(bruinwalkLink);
                        console.log('done adding a tag');
                    }
                }));
...
//@param name
//name from class planner. Usually format is "LAST, INITITAL FIRST. INITTIAL MID."
...
// return a url to bruinwalk class and professor page.
// return null if cannot find professor and class
function fetchInstBruinwalk(responseHTMLString, instNameArr){
    ...
    for(var i = 0; i < profNames.length; i++){
        ...
        if (lastName == instNameArr[0]){
            ...
            var instUrl = $(bwInstDiv).attr('href');
            return "https://az.bruinwalk.com" + instUrl;
        }
        else {
